...
format_orders1day<-function(df_tasks_for_today,clust_names){
  ...
  pred_M_shft<-df_tasks_for_today[as.POSIXct(paste(Date,From,sep= " "),format="%Y-%m-%d %H:%M") <as.POSIXct(paste(Date,"14:00",sep= " "),format="%Y-%m-%d %H:%M"),lapply(.SD,sum), by=Date,.SDcols=c( "Camden" ,"City" ,"North East" , "Soho" ,"South" , "Southeast" ,"Victoria" ,"West (new)",  "West",   "Zentrallager" ,"Central") ]
  pred_E_shft<-df_tasks_for_today[as.POSIXct(paste(Date,From,sep= " "),format="%Y-%m-%d %H:%M") >=as.POSIXct(paste(Date,"14:00",sep= " "),format="%Y-%m-%d %H:%M"), lapply(.SD,sum),by=Date,.SDcols=c( "Camden" ,"City" , "North East" , "Soho" ,"South" ,"Southeast" ,"Victoria" ,"West (new)", "West",    "Zentrallager" ,"Central") ]
...
idle_time_raw[idle_time_raw$fleet__name=="Nord Westen","fleet__name"]<-"Zentrallager"
idle_time_raw[idle_time_raw$fleet__name=="Zentral","fleet__name"]<-"Zentrallager"
idle_time_raw[idle_time_raw$fleet__name=="EBERTY-ZENTRAL","fleet__name"]<-"Zentrallager"
...
clust_names_table_9<-c("Wday","shift","Camden","Central","City","North East" ,"Soho", "South" ,"Southeast","Victoria","West (new)" ,"West","Zentrallager")
