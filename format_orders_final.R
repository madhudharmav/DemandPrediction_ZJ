...
format_realorders<-function(df_prediction_all,test_dates){
  ...
  pred_M_shft<-df_prediction_2daysahead[as.POSIXct(paste(Date,From,sep= " "),format="%Y-%m-%d %H:%M") <as.POSIXct(paste(Date,"14:00",sep= " "),format="%Y-%m-%d %H:%M"),lapply(.SD,sum), by=Date,.SDcols=c( "Camden" ,"City" ,"North East" , "Soho" ,"South" , "Southeast" ,"Victoria" ,"West (new)",  "West",   "EBERTY-ZENTRAL" ,"Central") ]
  pred_E_shft<-df_prediction_2daysahead[as.POSIXct(paste(Date,From,sep= " "),format="%Y-%m-%d %H:%M") >=as.POSIXct(paste(Date,"14:00",sep= " "),format="%Y-%m-%d %H:%M"), lapply(.SD,sum),by=Date,.SDcols=c( "Camden" ,"City" , "North East" , "Soho" ,"South" ,"Southeast" ,"Victoria" ,"West (new)", "West",    "EBERTY-ZENTRAL" ,"Central" ) ]
...
format_drivers_asd_utilrate_lon<-function(df_prediction_all,test_dates,df_test,avgtrvl_time_tslots,clust_names){
  library(data.table)
  ...
  table_12d<-utilrate_format_driv_london(table_10d)
  table_12<-cbind(Date=table_3[,Date],table_12d)
  return(table_12)
  
}

format_drivers_asd_utilrate_ber<-function(df_prediction_all,test_dates,df_test,avgtrvl_time_tslots,clust_names){
  library(data.table)
  ...
  table_10d[abs(table_10d)==Inf]<-NA
  table_10<-cbind(Date=table_3[,Date],table_10d)
  table_12d<-utilrate_format_driv_berlin(table_10d)
  table_12<-cbind(Date=table_3[,Date],table_12d)
  return(table_12)
...
format_drivers_asd_utilrate_par<-function(df_prediction_all,test_dates,df_test,avgtrvl_time_tslots,clust_names){
  library(data.table)
  ...
  table_10d[abs(table_10d)==Inf]<-NA
  table_10<-cbind(Date=table_3[,Date],table_10d)
  table_12d<-utilrate_format_driv_paris(table_10d)
  table_12<-cbind(Date=table_3[,Date],table_12d)
  table_12$East<-1
...
format_asd_values<-function(df_prediction_all,test_dates,df_test,avgtrvl_time_tslots,clust_names){
  library(data.table)
